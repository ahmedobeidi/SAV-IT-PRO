...
export function useRepairOrdersList(search: string, status: RepairStatus | "", page: number, limit: number) {
  const [data, setData] = useState<Paginated<RepairOrderRead> | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [nonce, setNonce] = useState(0);

  const key = useMemo(() => `${search}|${status}|${page}|${limit}|${nonce}`, [search, status, page, limit, nonce]);
